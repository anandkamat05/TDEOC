...
def WallEnvFactory(class_type):
    ...
    class WallEnv(class_type, utils.EzPickle):
        ...
        def __init__(
                ...
                model_path,
                ori_ind,
                wall_height = .12,   # For transfer change from 1.2 to 2.0
                wall_pos_range = ([10.0, 0.0], [10.0, 0.0]),
                n_bins=10,
                sensor_range=10.,
            ...
            self.wall_size = (0.15, 1., height)

            self.num_walls = 1
            self.space_between = 2.5
            self.walls_cleared = np.zeros(self.num_walls)
        ...
        def get_body_xquat(self, body_name):
            idx = self.model.body_names.index(six.u(body_name))
        ...
        def _is_over_wall(self, pos):
            x,y = pos
            l,b,h = self.wall_size
            for i in range(self.num_walls):
                minx = self.wall_pos[0] + i*self.space_between + l + 0.2
                maxx = self.wall_pos[0] + i*self.space_between + l + 0.4
                if minx <= x <= maxx and self.walls_cleared[i]==0:
                    self.walls_cleared[i] = 1
                    return True
            return False
...
def WallEnvFactorySensor(class_type):
    """class_type should be an OpenAI gym time"""
    ...
    class WallEnv(class_type, utils.EzPickle):
        ...
        def _get_obs(self):
            ...
            wall_length = self.wall_size[0]

            for i in range(self.num_walls):
                wall_startx = self.wall_pos[0]+i*self.space_between
                wall_endx = wall_startx + self.wall_size[0]
                diff = wall_startx - (robot_x)

                if diff > 0.:
                    start_index = (round(diff))
                    end_index = start_index + (wall_length)
                    if diff > self._sensor_range:
                        diff =0
                        continue
                    else:
                        break
...
######## Hurdle for Ant ############
def ProgressiveWallEnvFactory(class_type):
    """class_type should be an OpenAI gym time"""
    ...
    class WallEnv(class_type, utils.EzPickle):
        ...
        def __init__(
                ...
                ori_ind,
                wall_height = .12,
                wall_pos_range = ([0.8, 0.0], [0.8, 0.0]),
                n_bins=10,
                sensor_range=10.,
                sensor_span=math.pi/2,
                *args,
...
def IceWallFactory(class_type,):
    ...
    class IceWallEnv(class_type, utils.EzPickle):
        ...
        def __init__(
                ...
                ori_ind,
                wall_height = .12,
                wall_pos_range = ([2.8, 0.0], [2.8, 0.0]),  # For transfer change from 2.8 to 3.3
                n_bins=10,
                sensor_range=10.,
                sensor_span=math.pi/2,
                *args,
        ...
        def get_body_xquat(self, body_name):
            ...
            return self.sim.data.xquat[idx]
