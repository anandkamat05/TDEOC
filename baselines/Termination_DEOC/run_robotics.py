...
def train(env_id, num_timesteps, seed, num_options,app, saves ,wsaves, epoch,dc, render=False, caption='', deoc=False, tradeoff=0.1, term_mult=1.0, lr_mult=1.0, tdeoc=False):
    ...
    def policy_fn(name, ob_space, ac_space):
        return mlp_policy.MlpPolicy(name=name, ob_space=ob_space, ac_space=ac_space,
                                    hid_size=256, num_hid_layers=3, num_options=num_options, dc=dc)
    env.seed(seed)
...
def main():
    ...
    parser.add_argument('--deoc', help='Combine with pseudo reward with 2 networks', action='store_true', default=False)
    ...
    parser.add_argument('--tdeoc', help='Only use diversity in termination objective', action='store_true', default=False)
    ...
    train(args.env, num_timesteps=5e6, seed=args.seed, num_options=args.opt, app=args.app, saves=args.saves,
